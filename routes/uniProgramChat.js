...
const port = 3800;
const UniversityProgramRoom = require('../model/UniversityProgramRoom');
...
io.on("connection", socket => {
    ...
    socket.on('room', async data => {
        ...
        let roomId = '';
        const queryRoom = await UniversityProgramRoom.findOne({ name: data.room.roomName })
        if (!queryRoom) {
            console.log("q3: " + JSON.stringify(data.room))
            const newRoom = new UniversityProgramRoom({
    ...
    socket.on("chat message", async msg => {
        ...
        const queryRoom = await UniversityProgramRoom.findOne({ name: msg.room });
